...
def make_repositories_private(
    ...
    for repo_name in repo_names:
        with patch(
            ...
            if response.status_code == 200:
                print(f'Info: Repository "{repo_name}" | Changed to: Private')
            else:
                print(
                    f"Error ({response.status_code}): Private-ization Failed: "
                    f'"{repo_name}"'
                )
                print("Message:", response.text)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="GitHub Repositories Privatizer"
    )
    parser.add_argument(
        "-f",
        "--file",
        help="file containing repository names to make private (one per line)",
        type=str,
    )
    ...
    if args.file:
        ...
        if repo_names:
            make_repositories_private(username, access_token, repo_names)
        else:
            print("No Processing")
    else:
        confirm = input(
            "Continue (this'll make the repositories private)? (yes/no): "
        )
        if confirm.lower() != "yes":
            print("Exiting...")
            return

        public_repositories = list_public_repositories(username, access_token)
        if public_repositories:
            repo_names = [
                repository["name"] for repository in public_repositories
            ]
            make_repositories_private(username, access_token, repo_names)
